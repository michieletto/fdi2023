...
void first_prisoner(int chessboard[], int size, int magic_square) {
    // Compute the initial parity of the chessboard 
    // as set by the jailer and print its value for
    // information purposes
    int current_value = parity(chessboard, size);
    printf("Current parity: %d\n", current_value);
    // Compute the index of the coin to flip as the XOR
    // between the magic square and the parity of the chessboard
    // as set by the jailer at the beginning of the game
    // and print its value for information purposes
    int to_flip = magic_square ^ current_value;
    printf("To flip: %d\n", to_flip); 
    // Flip the coin at the given index (to_flip)
    // by using the flip function so that the
    // second prisoner can guess the magic square
    // just by computing the parity of the chessboard
    flip(chessboard, size, to_flip);
}
...
int second_prisoner(int chessboard[], int size) {
    // The second prisoner can guess the magic square
    // by computing the parity of the chessboard
    // since the first prisoner flipped a coin
    // to make the parity equal to the magic square index
    return parity(chessboard, size);
}

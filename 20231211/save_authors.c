...
Author* load_authors(char filename[], int *num_authors_ptr) {
    ...
    Author* authors_ptr = (Author*)(malloc((*num_authors_ptr+1)*sizeof(Author)));
...
bool save_authors(char filename[], Author *authors_ptr, int* num_authors_ptr) {
    // Open the text file in write mode
    // to overwrite the previous content
    // We cannot use "a" to append the content
    // because we need to update the number of authors
    ...
    // Write the number of authors to the file
    // Remember to add the new line character (\n)
    // to keep the same format of the file
    fprintf(file, "%d\n", *num_authors_ptr);

    // Write each author to the file
    for(int i=0; i<*num_authors_ptr; i++) {
        // Each field is separated by a space
        // and each author is separated by a new line character (\n)
        // You can use separate fprintf calls for each field
        fprintf(file, "%d ", authors_ptr[i].id);
        fprintf(file, "%s ", authors_ptr[i].name);
        fprintf(file, "%s ", authors_ptr[i].surname);
        fprintf(file, "%d ", authors_ptr[i].day);
        fprintf(file, "%d ", authors_ptr[i].month);
        fprintf(file, "%d\n", authors_ptr[i].year);
        // Or you can use a single fprintf call for each author
        // and use the format string to specify the format of each field
        // fprintf(file, "%d %s %s %d %d %d\n", authors_ptr[i].id,
        //                                      authors_ptr[i].name, 
        //                                      authors_ptr[i].surname, 
        //                                      authors_ptr[i].day,
        //                                      authors_ptr[i].month,
        //                                      authors_ptr[i].year);
    }
...
int main() {
    ...
    char filename[] = "../20231204/data/authors.txt";
    ...
    read_author(authors_ptr, &num_authors);

    // Save the authors in the file
    if(!save_authors(filename, authors_ptr, &num_authors)) {
        // Print an error message
